...
- SCL-bound Sampled Values payload decoder for PCAP/passive subscriber
  workflows, including `smpCnt` wrap, gap, missed-sample, duplicate, and
...
- 138 automated tests passing in the latest local validation run.
...
- live GOOSE/SV subscriber receive loops on raw NICs;
- multi-ASDU SV publisher/subscriber handling;
...
dotnet run --project .\apps\AR.Iec61850.Cli -- stream-pcap .\out\processbus-demo.pcap --scl .\samples\scl\minimal-station.scd --delay-ms 50 --limit 12
```
...
```powershell
dotnet run --project .\apps\AR.Iec61850.Cli -- publish-sv-live ".\samples\scl\01_SV_Stream_4I+4V_(9-2LE).scd" --adapter 1 --stream-index 1 --frames 4000 --smpcnt-wrap auto --dry-run
```
...
### Golden SCL registry promotion

`mms-service-discover` can now turn golden SCL learning results into a machine-readable CDC profile layer:

```powershell
dotnet run --project .\apps\AR.Iec61850.Cli -- mms-service-discover 192.16.1.157 --port 102 --read-types false --learn-types-from-golden true --golden-scl .\samples\scl\OCR7SR12.iid --golden-profile-name OCR7SR12 --golden-learning-conflict-policy review-only --ied-name OCR7SR12 --output out\service-discovery\OCR7SR12
```

New outputs:

- `golden-learning-registry-promotion.json`
- `golden-learning-registry-promotion.md`
- `golden-learned-cdc-registry.json`

The default policy is `review-only`, so CDC conflicts are recorded for engineering review instead of silently overriding live discovery.
