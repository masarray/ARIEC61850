...
## N5.16 — Golden Learning Registry Promotion + Vendor Profile

- Convert golden SCL type-learning candidates into an auditable CDC registry promotion plan.
- Emit `golden-learning-registry-promotion.json/.md` and `golden-learned-cdc-registry.json` during `mms-service-discover`.
- Support `--golden-profile-name` and `--golden-learning-conflict-policy review-only|prefer-live|prefer-golden`.
- Default conflict behavior is review-only: high/exact live inference is never overridden without an explicit policy.
- Use the generated registry as the next input for CDC registry expansion and SCL normalizer work.

## N5.17 - Smart SV Publisher Payload Layout

- Add a core `SampledValuesPayloadLayout` model that maps SCL DataSet entries to SV raw payload offsets, widths, and supported element kinds.
- Add `SampledValuesPayloadBuilder` for typed big-endian SV data-block generation from MMS-style values and deterministic demo waveforms.
- Promote CLI `publish-sv-live` from app-local byte generation to core layout-driven payload generation.
- Add automatic `smpCnt` wrap resolution from `smpRate`/`smpMod` with CLI override `--smpcnt-wrap auto|none|N`.
- Fail explicitly for `nofASDU > 1` until multi-ASDU publishing is implemented, avoiding semantically wrong one-ASDU output.
- Reuse the same payload layout engine in the WPF SV Publisher instead of maintaining a separate 4-byte-per-entry generator.
- Remaining gap: subscriber-side typed payload decode, multi-ASDU publisher sessions, engineering-unit scaling, and protection-grade pacing/timing evidence.

## N5.18 - SCL-bound SV Subscriber Decode + Sequence Diagnostics

- Add `SampledValuesPayloadDecoder` as the receive-side pair of the N5.17 payload builder.
- Decode raw SV sample buffers back into typed MMS-style values using SCL DataSet order, offsets, widths, and bType-derived element kinds.
- Bind `ProcessBusStreamMonitor` to optional SCL `SampledValueControl` profiles so PCAP replay can distinguish anonymous payload bytes from engineering-aware decoded values.
- Track per-stream `smpCnt` state: expected next count, profile-aware wrap, jump/loss, missed sample count, duplicate count, and out-of-order/late frames.
- Extend `inspect-pcap` and `stream-pcap` with `--scl` and `--nominal-hz` for passive SCL-aware SV diagnostics.
- Remaining gap: live raw-NIC SV subscriber receive loop, multi-ASDU decode/publish, engineering-unit scaling, PTP/hardware timestamp evidence, and R-SMV/IEC 61850-90-5.
