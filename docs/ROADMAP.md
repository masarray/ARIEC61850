...
IED Discovery moves from shell to a more usable engineering browser. The middle detail panel now behaves as a lightweight tree-grid over a flat virtualized row source, so nested MMS structures can be expanded without forcing DA-level noise into the left explorer. Enable RCB opens a dedicated report dialog instead of a preview popup, validates DataSet selection, and starts the existing guarded report workflow when the plan is ready.
