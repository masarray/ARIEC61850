...
$ForbiddenDirectories = @(
    ...
    "evidence", "captures", "pcaps", "reports", "logs", "TestResults"
)
$ForbiddenFilePatterns = @(
    "*.dll", "*.exe", "*.pdb", "*.deps.json", "*.runtimeconfig.json",
    "*.pcap", "*.pcapng", "*.etl", "*.binlog", "*.suo", "*.user", "*.rsuser"
)
$ForbiddenTextPatterns = @(
    "ARIEC60870", "IEC60870", "IEC 60870", "IEC101", "IEC103", "IEC104",
    "libiec61850", "MZ Automation", "General Public License", "OCR7SR12", "OMICRON_CMC"
)
...
$TextFiles = Get-ChildItem -Path $RepoRoot -Recurse -Force -File -Include *.md,*.cs,*.xml,*.xaml,*.ps1,*.cmd,*.yml,*.yaml,*.html,*.css,*.js,*.json,*.props,*.sln,*.slnx,*.txt -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -notmatch "\\(bin|obj|.vs|.artifacts|artifacts|out)\\" }
