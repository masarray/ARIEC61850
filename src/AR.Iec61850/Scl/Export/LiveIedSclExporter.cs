...
public static class LiveIedSclExporter
{
    ...
    public static XDocument BuildDocument(LiveIedModelDiscoveryDocument model, LiveIedSclExportOptions? options = null)
    {
        ...
        AddSchemaCompatibilityWarnings(model, options, context);
        return BuildDocumentWithContext(model, options, context);
    }
    ...
    {
        var schema = options.ResolvedSchemaProfile;
        var root = new XElement(
            Scl + "SCL",
            OptionalAttribute("version", schema.RootVersion),
            OptionalAttribute("revision", schema.RootRevision),
        ...
        return new XDocument(
            ...
            new XComment("Generated by ARIEC61850 from live MMS discovery."),
            new XComment($"SCL Schema Version {schema.SchemaVersion} ({schema.SchemaDate})"),
            root);
    }
    ...
    private static LiveIedSclExportResult BuildReport(
        ...
        => new()
        {
            ...
            SclSchema = options.ResolvedSchemaProfile.DisplayName,
            SclPath = sclPath,
    ...
    private static void AddSchemaCompatibilityWarnings(
        LiveIedModelDiscoveryDocument model,
        LiveIedSclExportOptions options,
        LiveIedSclBuildContext context)
    {
        var schema = options.ResolvedSchemaProfile;
        if (schema.SupportsTriggerGi || model.ReportControls.Count == 0)
            return;

        context.Warnings.Add(new LiveIedSclExportWarning
        {
            Code = "SchemaDowngradeTriggerGi",
            Reference = schema.DisplayName,
            Message = "TrgOps.gi is omitted for the selected Edition 1 schema. The live RCB evidence remains available in the companion export report."
        });
    }
        ...
        LiveIedSclExportOptions options)
    {
        ...
        {
            ied.Add(new XComment("Generated from live MMS discovery. Runtime states such as RptEna, ResvTms, Owner, SqNum, EntryID, and contention evidence are intentionally not encoded as static SCL configuration."));
        }

        ied.Add(BuildServices(model, options));
        ied.Add(new XElement(
            Scl + "AccessPoint",
    ...
    private static XElement BuildServices(LiveIedModelDiscoveryDocument model, LiveIedSclExportOptions options)
    {
        var schema = options.ResolvedSchemaProfile;
        var services = new XElement(
            Scl + "Services",
            new XElement(Scl + "DynAssociation"),
            new XElement(Scl + "GetDirectory"),
            new XElement(Scl + "GetDataObjectDefinition"),
            new XElement(Scl + "DataObjectDirectory"));

        if (model.DataSets.Count > 0)
        {
            services.Add(new XElement(Scl + "DataSetDirectory"));
            services.Add(new XElement(
                Scl + "ConfDataSet",
                new XAttribute("max", model.DataSets.Count.ToString(CultureInfo.InvariantCulture)),
                new XAttribute("modify", "false")));
        }

        if (model.SettingGroupControls.Count > 0)
        {
            services.Add(new XElement(
                Scl + "SettingGroups",
                new XElement(Scl + "SGEdit", schema.SupportsReservationTime ? new XAttribute("resvTms", "true") : null),
                new XElement(Scl + "ConfSG", schema.SupportsReservationTime ? new XAttribute("resvTms", "true") : null)));
        }

        if (model.ReportControls.Count > 0)
        {
            services.Add(new XElement(
                Scl + "ConfReportControl",
                new XAttribute("max", model.ReportControls.Count.ToString(CultureInfo.InvariantCulture)),
                schema.IsEdition2 ? new XAttribute("bufConf", "false") : null));
            services.Add(new XElement(Scl + "GetCBValues"));
            services.Add(new XElement(
                Scl + "ReportSettings",
                new XAttribute("rptID", "Dyn"),
                new XAttribute("optFields", "Dyn"),
                new XAttribute("bufTime", "Dyn"),
                new XAttribute("trgOps", "Dyn"),
                new XAttribute("intgPd", "Dyn"),
                schema.SupportsReservationTime ? new XAttribute("owner", "true") : null,
                schema.SupportsReservationTime ? new XAttribute("resvTms", "true") : null));
        }

        services.Add(new XElement(
            Scl + "ConfLNs",
            new XAttribute("fixPrefix", "true"),
            new XAttribute("fixLnInst", "true")));
        services.Add(new XElement(
            Scl + "GOOSE",
            new XAttribute("max", model.GooseControlBlocks.Count.ToString(CultureInfo.InvariantCulture))));
        services.Add(new XElement(Scl + "GSSE", new XAttribute("max", "0")));

        if (model.FileDirectory.IsSuccess)
            services.Add(new XElement(Scl + "FileHandling"));
        ...
        bool isLn0)
    {
        ...
        foreach (var rcb in model.ReportControls.Where(x => string.Equals(x.Domain, ldDomain, StringComparison.OrdinalIgnoreCase) && SameLogicalNode(x.LogicalNode, ln.Name)).OrderBy(x => x.Name, StringComparer.OrdinalIgnoreCase))
            element.Add(BuildReportControl(model, rcb, context, options));
        ...
        LiveIedSclExportOptions options)
    {
        ...
        var element = new XElement(
            ...
            BuildTriggerOptions(rcb.TriggerOptions, options.ResolvedSchemaProfile),
    ...
    private static XElement BuildTriggerOptions(string source, SclSchemaProfileDescriptor schema)
    {
        var normalized = source ?? string.Empty;
        var element = new XElement(
            Scl + "TrgOps",
            new XAttribute("dchg", ContainsAny(normalized, "data-change", "dchg") ? "true" : "false"),
            new XAttribute("qchg", ContainsAny(normalized, "quality-change", "qchg") ? "true" : "false"),
        ...
        if (schema.SupportsTriggerGi)
            element.Add(new XAttribute("gi", ContainsAny(normalized, "gi", "general-interrogation") ? "true" : "true"));
    ...
    private static XElement BuildOptionalFields(string source)
    {
        ...
        return new XElement(
            Scl + "OptFields",
            new XAttribute("seqNum", ContainsAny(normalized, "sequence-number", "seqnum") ? "true" : "false"),
            new XAttribute("timeStamp", ContainsAny(normalized, "report-time-stamp", "timestamp", "timeofentry") ? "true" : "false"),
            new XAttribute("reasonCode", ContainsAny(normalized, "reason", "reason-for-inclusion") ? "true" : "false"),
            new XAttribute("dataSet", ContainsAny(normalized, "data-set-name", "dataset") ? "true" : "false"),
            new XAttribute("dataRef", ContainsAny(normalized, "data-reference", "dataref") ? "true" : "false"),
            new XAttribute("entryID", ContainsAny(normalized, "entryid", "entryID") ? "true" : "false"),
            new XAttribute("configRef", ContainsAny(normalized, "conf-revision", "configref") ? "true" : "false"));
    }
    ...
    private static string BuildMarkdown(LiveIedModelDiscoveryDocument model, LiveIedSclExportResult result)
    {
        ...
        sb.AppendLine($"- Schema: {Escape(result.SclSchema)}");
        sb.AppendLine();
    ...
    private static void AddAddressP(XElement address, string type, string value)
    {
        ...
        address.Add(new XElement(
            Scl + "P",
            new XAttribute("type", type),
            string.Equals(type, "OSI-AP-Title", StringComparison.OrdinalIgnoreCase)
                ? null
                : new XAttribute(Xsi + "type", $"tP_{type}"),
            value.Trim()));
    }
