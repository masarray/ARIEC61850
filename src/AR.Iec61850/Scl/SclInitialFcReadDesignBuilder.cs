...
public static class SclInitialFcReadDesignBuilder
{
    ...
    public static SclInitialFcReadDesign Read(XDocument document, string iedName, string accessPointName)
    {
        ...
        // with the same LDevice@inst can never contribute the trusted online shape.
        ...
        // DataSet membership and ReportControl identity are as authoritative as the DA
        // shape for the trusted-SCL path. Preserve them in the same scoped model instead
        // of dropping them during FC-read projection. References are remapped to ldName
        // using the exact same domain map as LogicalDevices.
